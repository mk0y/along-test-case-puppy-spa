...
export class WaitingListEntryService {
  ...
  async create(createWaitingListEntryInput: CreateWaitingListEntryInput, waitingListId: number): Promise<WaitingListEntry> {
    const waitingList = await this.entriesRepo.findOneBy({ id: waitingListId });
  ...
  async updatePosition(id: number, newPosition: number): Promise<WaitingListEntry> {
